...
export default function ArtistClient({ artist }) {
  const { t, tl } = useLang();
  ...
  const { name, genres, bio, photo_url, social, gallery, youtube, set: setData } = artist;
    ...
    <>
      ...
      <main className="min-h-screen pt-32 pb-12 px-6 md:px-12 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-12 gap-12 lg:gap-20 items-start">
            <div className="lg:col-span-5 relative group w-full">
              <div className="aspect-[3/4] overflow-hidden rounded-xl bg-surface-container-low shadow-2xl relative">
                <Image className="object-cover transition-transform duration-700 group-hover:scale-105" src={photo_url} alt={name} fill sizes="(max-width: 1024px) 100vw, 40vw" quality={95} priority unoptimized />
            ...
            <div className="lg:col-span-7 flex flex-col gap-8">
              ...
              <div className="space-y-6 max-w-2xl">
                ...
                <p className="text-on-surface-variant leading-relaxed text-lg font-light tracking-wide whitespace-pre-line">{tl(artist, "bio")}</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-6 mt-4">
                ...
                <button onClick={() => { setPwOpen(true); setPwValue(""); setPwError(false); }} className="flex items-center gap-3 text-white border border-white/10 px-8 py-4 rounded-xl font-headline font-bold uppercase tracking-widest hover:bg-white/5 transition-all">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 pt-12 border-t border-white/5 mt-auto">
                ...
                <div>
                  ...
                  <div className="font-headline text-sm text-white">{artist.labels?.join(" | ") || "UTOPIC"}</div>
                </div>
                <div>
          ...
          {(gallery?.length > 0 || setData || youtube) && (
              ...
              <div className="grid md:grid-cols-2 gap-12">
                ...
                {(setData || youtube) && (
                  <div>
                    ...
                    {setData?.type === "soundcloud" ? (
                      <div className="rounded-xl overflow-hidden bg-surface-container-low">
                        <iframe width="100%" height="300" scrolling="no" frameBorder="no" allow="autoplay" src={`https://w.soundcloud.com/player/?url=${encodeURIComponent(setData.url)}&color=%234da6ff&auto_play=false&hide_related=true&show_comments=false&show_user=true&show_reposts=false&show_teaser=false`} />
                      </div>
                    ) : (
                      <div className="aspect-video rounded-xl overflow-hidden bg-surface-container-low">
                        <iframe src={setData?.url || youtube} className="w-full h-full" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen loading="lazy" title={`${name} set`} />
                      </div>
                    )}
                  </div>
                )}
      ...
      {pwOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-6" onClick={() => setPwOpen(false)}>
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm" />
          <div className="relative bg-background border border-white/10 rounded-2xl p-8 w-full max-w-sm shadow-[0_0_60px_rgba(77,166,255,0.15)]" onClick={(e) => e.stopPropagation()}>
            <h3 className="font-headline text-lg font-black text-white uppercase tracking-tight mb-4">Presskit Access</h3>
            <input
              type="password"
              value={pwValue}
              onChange={(e) => { setPwValue(e.target.value); setPwError(false); }}
              onKeyDown={(e) => { if (e.key === "Enter" && pwValue === "UTPWW26!") { setPwOpen(false); window.open(artist.presskit, "_blank"); } else if (e.key === "Enter") setPwError(true); }}
              placeholder="Password"
              className="w-full bg-surface-container-low border border-white/10 rounded-xl px-5 py-4 text-white font-body text-sm placeholder:text-white/30 focus:outline-none focus:border-primary/50 transition-colors mb-3"
              autoFocus
            />
            {pwError && <p className="text-red-400 text-xs mb-3">Incorrect password</p>}
            <button
              onClick={() => { if (pwValue === "UTPWW26!") { setPwOpen(false); window.open(artist.presskit, "_blank"); } else setPwError(true); }}
              className="w-full py-3 bg-primary text-on-primary-fixed font-headline font-bold uppercase tracking-widest text-xs rounded-xl hover:shadow-[0_0_30px_rgba(77,166,255,0.4)] transition-all"
            >
              Access
            </button>
          </div>
